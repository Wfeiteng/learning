...
public class StringToIntegerAtoi8 {
    ...
    class Solution {
        public int myAtoi(String str) {
            ...
            int filter = 0;
            for (; filter < bytes.length; filter++) {
                char value = bytes[filter];
                if (value == ' ') {
                    continue;
                } else if (value == '-') {
                    flgvalue = -1;
            ...
            for (int first = filter; first < bytes.length; first++) {
                char value = bytes[first];
                if (isDigit(value)) {
                    int tmp = value - '0';
                    if (result > (Integer.MAX_VALUE - tmp) / 10) {
                        return flgvalue == -1 ? Integer.MIN_VALUE : Integer.MAX_VALUE;
                    }
